...
def format_pf_non_streaming_response(
    ...
    try:
        ...
        if response_field_name in chatCompletion:
            messages.append({
                "role": "assistant",
                "content": chatCompletion[response_field_name].get("content") if chatCompletion[response_field_name].get("content") else chatCompletion[tool_response_name].get("tool_response")
            })
